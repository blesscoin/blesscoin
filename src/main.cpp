...
uint256 hashGenesisBlock("0xe2b9c4de5048fe6bce794c1cc5e70e5f35f4c968a44bca559bc2512eafabee35");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Blesscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Blesscoin Signed Message:\n";
...
{
    // Blesscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Blesscoin: 1000k blocks in ~4 years

    // premined 2% for  dev, support, bounty, and giveaway etc
    if(nHeight == 1){
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Blesscoin: 1 days
static const int64 nTargetSpacing = 60; // Blesscoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Blesscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Blesscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Blesscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "God Bless You Now";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388369621;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 387423989;
        ...
        assert(block.hashMerkleRoot == uint256("0x3cd3e424ab4d590b437c275fe892fe0a58850daf943674249e585af53d31fa9b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Blesscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlesscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BlesscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BlesscoinMiner(CWallet *pwallet)
{
    printf("BlesscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blesscoin-miner");
    ...
    try { loop {
        ...
        printf("Running BlesscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BlesscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlesscoinMiner, pwallet));
}
